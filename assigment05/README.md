...
* `public/index.ts.html` is the page template;
* `src/index.ts.js` is the JavaScript entry point.
...
Only files inside `public` can be used from `public/index.ts.html`.<br>
...
If you are concerned about using Webpack-specific semantics, you can put all your CSS right into `src/index.ts.css`. It would still be imported from `src/index.ts.js`, but you could always remove that import if you later migrate to a different build tool.
...
Inside `index.ts.html`, you can use it like this:
...
Import Bootstrap CSS and optionally Bootstrap theme CSS in the beginning of your ```src/index.ts.js``` file:
...
You can optionally use an IDE like [Nuclide](https://nuclide.io/docs/languages.ts/flow/) for a better integrated experience.
...
You can also access the environment variables starting with `REACT_APP_` in the `public/index.ts.html`. For example:
...
```
/             - static server returns index.ts.html with React app
/todos        - static server returns index.ts.html with React app
...
Then, on the server, regardless of the backend you use, you can read `index.ts.html` into memory and replace `__OG_TITLE__`, `__OG_DESCRIPTION__`, and any other placeholders with values depending on the current URL. Just make sure to sanitize and escape the interpolated values so that they are safe to embed into HTML!
...
* [`mount()`](http://airbnb.io/enzyme/docs/api/mount.html) in [Enzyme](http://airbnb.io/enzyme/index.ts.html)

In contrast, **jsdom is not needed** for the following APIs:

* [`TestUtils.createRenderer()`](https://facebook.github.io/react/docs/test-utils.html#shallow-rendering) (shallow rendering)
* [`shallow()`](http://airbnb.io/enzyme/docs/api/shallow.html) in [Enzyme](http://airbnb.io/enzyme/index.ts.html)
...
from [`src/index.ts.js`](src/index.ts.js).

If you had previously enabled service workers in your production deployment and
have decided that you would like to disable them for all your existing users,
you can swap out the call to `serviceWorkerRegistration.register()` in
[`src/index.ts.js`](src/index.ts.js) with a call to `serviceWorkerRegistration.unregister()`.
...
`npm run build` creates a `build` directory with a production build of your app. Set up your favorite HTTP server so that a visitor to your site is served `index.ts.html`, and requests to static paths like `/static/js/main.<hash>.js` are served with the contents of the `/static/js/main.<hash>.js` file.
...
app.get('/', function (req, res) {
  res.sendFile(path.join(__dirname, 'build', 'index.ts.html'));
});
...
This is because when there is a fresh page load for a `/todos/42`, the server looks for the file `build/todos/42` and does not find it. The server needs to be configured to respond to a request to `/todos/42` by serving `index.ts.html`. For example, we can amend our Express example above to serve `index.ts.html` for any unknown paths:
...
```
    ...
    RewriteRule ^ index.ts.html [QSA,L]
```
...
`/todos/42`, by serving the cached copy of your `index.ts.html`. This
...
This will make sure that all the asset paths are relative to `index.ts.html`. You will then be able to move your app from `http://mywebsite.com` to `http://mywebsite.com/relativepath` or even `http://mywebsite.com/relative/path` without having to rebuild it.
...
```sh
    ...
    ? Configure as a single-page app (rewrite all urls to /index.ts.html)? Yes
    ✔  Wrote build/index.ts.html
...
* Alternatively, you can use a trick to teach GitHub Pages to handle 404 by redirecting to your `index.ts.html` page with a special redirect parameter. You would need to add a `404.html` file with the redirection code to the `build` folder before deploying your project, and you’ll need to add code handling the redirect parameter to `index.ts.html`. You can find a detailed explanation of this technique [in this guide](https://github.com/rafrex/spa-github-pages).
...
Note that in order to support routers that use HTML5 `pushState` API, you may want to rename the `index.ts.html` in your build folder to `200.html` before deploying to Surge. This [ensures that every URL falls back to that file](https://surge.sh/help/adding-a-200-page-for-client-side-routing).
...
* If the watcher doesn’t see a file called `index.ts.js` and you’re referencing it by the folder name, you [need to restart the watcher](https://github.com/facebookincubator/create-react-app/issues/1164) due to a Webpack bug.
